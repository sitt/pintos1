...
/* A sleeping_threads_list element. It consists of
   a pointer to the sleeping thread together with
   the encoding of the moment it should wake up (in timer
   ticks since OS booted). */
struct sleeping_threads_element
  {
    struct semaphore s;
    struct list_elem elem;              /* List element. */
    struct thread *thread;              /* The sleeping thread. */
    int64_t wake_up_ticks;              /* Wake up time in OS ticks. */
  };
...
static void wake_up_sleeping_processes (void);
...
timer_sleep (int64_t ticks)
{
  ...
  /* Allocate and initialize sleeping thread's information. */
  sleeping_thread = malloc (sizeof *sleeping_thread);
  if (sleeping_thread == NULL)
  PANIC ("Failed to allocate memory for thread sleep accounting"
         "data structure.");
  sleeping_thread->thread = thread_current ();
  sleeping_thread->wake_up_ticks = timer_ticks () + ticks;

  sema_init(&sleeping_thread->s,0);
  sleep = &sleeping_thread->s;


  //sema_down(&sleeping_thread->s);
...
/* Helper function for inserting with priority sleeping_thread_element's
   to the sleeping_threads_list priority queue. Returns true if A
...
sleeping_thread_element_less (const struct list_elem *a,
                              ...
                              void *aux UNUSED)
{
  struct sleeping_threads_element *sleeping_thread_a
    = list_entry (a, struct sleeping_threads_element, elem);
  struct sleeping_threads_element *sleeping_thread_b
    = list_entry (b, struct sleeping_threads_element, elem);
...
static void
wake_up_sleeping_processes (void)
{
  struct sleeping_threads_element *sleeping_thread;
  ...
  while (!list_empty (&sleeping_threads_list) && (list_entry (list_front (&sleeping_threads_list), struct sleeping_threads_element, elem))->wake_up_ticks <= timer_ticks ())
    {
      /* Put into ready state and update sleeping threads' queue. */
      sleeping_thread = list_entry (list_pop_front (&sleeping_threads_list),
                                    struct sleeping_threads_element, elem);
